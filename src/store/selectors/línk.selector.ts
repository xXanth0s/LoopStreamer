...
export function getLinksForSeriesSeasonAndPortal(state: StateModel,
                                                 seriesSeasonKey: SeriesSeason['key'], portal: PORTALS): LinkModel[] {
...
export function isSeasonUpToDate(state: StateModel,
                                 seriesSeasonKey: SeriesSeason['key'], portal: PORTALS, language: LANGUAGE): boolean {
    const episode = getSeriesEpisodesForSeason(state, seriesSeasonKey);
    const links = getLinksByKeys(state, episode.flatMap(_episode => _episode.portalLinks));
...
export function getPortalLinksForSeriesEpisodePortalAndLanguage(state: StateModel,
                                                                seriesEpisodeKey: SeriesEpisode['key'],
                                                                portal: PORTALS,
                                                                language: LANGUAGE): LinkModel[] {
    const episode = getSeriesEpisodeByKey(state, seriesEpisodeKey);
